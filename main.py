...
def clean_ytdl_options():
    ...
    # Use YouTube authentication if available
    if YOUTUBE_AUTH_AVAILABLE:
        try:
            authenticated_options = clean_ytdl_options_with_auth()
            # Merge authenticated options with base options
            base_options.update(authenticated_options)
            logger.info("Using authenticated YouTube options")
        except Exception as e:
            logger.warning(f"Failed to get authenticated options, using base options: {e}")
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(options) as ydl:
                ...
                for entry in search_results['entries']:
                    ...
                    thumbnail = extract_best_thumbnail(entry)
                    ...
                    video = ensure_thumbnail_availability(video)
                    videos.append(video)
    ...
    async def get_details(url, video_id=None):
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(options) as ydl:
                ...
                # Ensure thumbnail availability
                return ensure_thumbnail_availability(video_data)
        except Exception as e:
